...
#### 카플랫 B2B 서비스 Web/Admin 구축

- Next.js App Router 기반 B2B 미들웨어 Web/Admin 개발 환경 초기 구성
- 차량 조회, 제어, 촬영 이미지 제출 등 운영 핵심 플로우 구현
- Kakao Map에서 수천 대 차량 마커 렌더링 시 지도 이동 중 프레임 저하가 발생해,
  zoom/viewport 기준 클러스터링을 적용하고 60fps 수준의 지도 이동 성능 유지
- AWS CloudWatch + RUM을 도입해 Next.js 앱 오류 추적 및 사용자 환경 모니터링 체계 구성

#### 카플랫 APP WebView / Native Bridge 구축

- 촬영 이미지 다중 업로드 및 API 제출 흐름 구현
- Native Bridge(BLE) 응답 지연/실패 케이스에 대응하기 위해 timeout/error 기준의 Web API fallback 차량 제어 흐름 구현
- 화면 전환, Bottom Sheet 인터랙션, 이미지 압축 등 모바일 웹뷰 UX 개선

#### B2B 파트너 온보딩을 위한 개발자센터 구축

- https://developers.carplat.co.kr
- 기획, 디자인을 포함해 1인 프론트엔드로 개발자센터 구축
- OpenAPI YAML을 단일 소스로 사용해 API 문서 자동 생성 구조 설계
- Endpoint, Parameter, 에러코드 통합 검색을 제공해 파트너 API 탐색 흐름 개선

#### 디자인 시스템 구축

- Radix UI 기반 primitive 위에 기본 컴포넌트 40개 및 복합 공용 컴포넌트 설계
- 전 프로젝트에서 디자인 시스템을 적용해 공통 컴포넌트 재구현 방지
- Storybook으로 문서화해 개발자, 디자이너, 기획자가 함께 참조할 수 있는 협업 기준 마련
- Figma Variables를 CSS Variable, Tailwind Utility로 변환하는 자동 생성 흐름 마련
- Storybook 문서를 Agent Skills로 변환해 AI 코드 생성 시 사내 컴포넌트 사용 정확도 개선

#### OpenAPI 기반 타입 작성 및 SDK 자동화

- OpenAPI 명세 기반 타입을 수동 작성하며 발생하는 반복 작업과 타입 불일치를 줄이기 위해,
  OpenAPI YAML을 단일 소스로 사용해 TypeScript 타입 자동 생성 및 Full Type Safe SDK 구현
- API 명세 변경 시 프론트엔드 타입과 호출부가 함께 갱신되도록 구성해 수동 타입 작성과 연동 실수 방지
...
- SEO 개선을 위한 메타데이터, json-ld, 시맨틱 마크업 구조 정비
